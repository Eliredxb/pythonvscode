"""
Daniel is thrilled to explore the Roblox store and enhance his gaming experience. He wants to purchase the game, select a game pass, buy some avatar outfits, and maybe even get a pet! Additionally, he’s considering buying some Robux to unlock more features. Write a Python program to help Daniel calculate the total cost of his Roblox adventure.
...
After making his selections, Daniel wants to see the total cost of his Roblox adventure. Let’s create a Python program to help him out.
"""
...
download = st.selectbox("Download roblox",["Choose","No(Exit)","Yes:150"])
if download == "No(Exit)":
...
if download == "Yes:150":
    cost =+ 150
    st.success("You bought roblox")
    st.balloons()
    name = st.text_input("Whats your username: ")
    password = st.text_input("Whats your password: ",type="password")
    palss = st.selectbox("Choose a pass",["VIP:50","Premium:120"])
    if palss == "VIP:50":
        cost =+ 50
    ...
    pet = st.selectbox("Do you want a pet",["No","Yes:40"])
    if pet == "Yes:40":
       cost =+ 40
       st.success =("You bought a pet")
    robux = st.selectbox("How much robux do you want to buy",["800robux:10","2000robux:25","4500robux:50"])
    if robux == "800robux:10": 
       cost =+ 10
       st.success("You bought 800 robuxs")

    if robux == "2000robux:25":
       cost =+ 25
       st.success("You bought 2000 robuxs")

    if robux == "4500robux:50":
       cost =+ 10
       st.success("You bought 4500 robuxs")
    
    if st.button("Show me cost"):
        st.success(f"The cost is {cost}")

    if palss == "Premium:150":
        cost =+ 50
        skins = st.selectbox("Choose a skin",["Superman:$20","Sonic:$30","Tails:$20"])
        if skins == "Superman:$20":
            cost += 20
    ...
    if st.button("Show my cost"):
        st.success(f"Your cost is {cost}")
